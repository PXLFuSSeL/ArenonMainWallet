...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "Arenon is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "Arenon is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Arenon is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Arenon is downloading blocks...");
