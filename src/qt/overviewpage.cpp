...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeArenonAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeArenonAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeArenonAmount = strAnonymizeArenonAmount.remove(strAnonymizeArenonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArenonAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeArenonAmount*COIN) nMaxToAnonymize = nAnonymizeArenonAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeArenonAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeArenonAmount));
        strAnonymizeArenonAmount = strAnonymizeArenonAmount.remove(strAnonymizeArenonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArenonAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
